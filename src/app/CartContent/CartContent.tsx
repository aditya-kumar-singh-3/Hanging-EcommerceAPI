...
const CartContent = () => {
  const cartInfo = useSelector((state: RootState) => state.cart.cartData)
  ...
  if (cartInfo.length === 0) {
    return (
     ...
     <Link href='/'>   <button className="bg-blue-600 text-white py-2 px-6 rounded-md hover:bg-blue-700 transition-colors">
          Continue Shopping
        </button></Link>
      </div>
    )
  ...
  function deleteitem(id : number){
    const data = cartInfo.find((item)=> item.id === id);
    if(data){
          ...
          {cartInfo.map((item) => (
            <div key={item.id} className="flex items-center border-b border-gray-200 py-4">
              <div className="flex-shrink-0 w-24 h-24 bg-gray-100 rounded-md overflow-hidden">
                <img src={item.image} alt={item.name} width={96} height={96} className="w-full h-full object-fit" />
              </div>
